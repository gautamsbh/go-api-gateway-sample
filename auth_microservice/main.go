...
func (a *App) Initialize(user, password, dbname string, host string) {
	...
	a.Router.HandleFunc("/auth", a.authenticate).Methods("GET")
}

func (a *App) authenticate(w http.ResponseWriter, r *http.Request) {
	username := r.Header.Get("username")
	log.Printf("hello %v", username)
	...
	respondWithJSON(w, http.StatusOK, map[string]string{"message": "you are successfully authenticated"})
}
...
func (a *App) Run(addr string) {
	log.Printf("Auth microservice starting on port auth_microservice:%v", addr)
